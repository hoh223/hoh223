...
for tol_idx = 1:length(tol)
    ...
    for basis_case = 1:2
        ...
        title(['Comparison of Computation Time for SASK and Gradient Descent (tol = ', num2str(tols), ', basis = ', basis_label, ')']);
        legend show;
        grid on;
    end
end
...
function y = compute_sum(xx)
    d = length(xx); % Get the length of the input vector
    sum = 0; % Initialize sum to 0

    for ii = 1:d
        xi = xx(ii); % Get the ii-th element of xx
        new = (abs(xi))^(ii + 1); % Compute the absolute value of xi raised to the power of (ii + 1)
        sum = sum + new; % Add the computed value to sum
    end

    y = sum; % Output the result
end

function grad = dcompute_sum(xx)
    d = length(xx); % Get the length of the input vector
    grad = zeros(size(xx)); % Initialize the gradient vector to zeros

    for ii = 1:d
        xi = xx(ii); % Get the ii-th element of xx
        grad(ii) = (ii + 1) * (abs(xi))^(ii) * sign(xi); % Compute the gradient for the ii-th element
    end
end
